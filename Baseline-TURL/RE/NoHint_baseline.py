    ...
    for id, example in enumerate(tqdm(examples)):
        ...
        messages=[
                ...
                },
                {
                    ...
                    4. Choose the single best-fitting relation from the label set that applies to all rows.
                    5. The chosen relation must be exactly one string from the list. Output only that relation in valid JSON format.
                    """,
                }
            ]
        
        chatgpt_msg = response(messages, "gpt-4o-mini")
